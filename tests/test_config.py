...
def test_network_requires_at_least_three_cores():
    with pytest.raises(ValueError):
        NetworkConfig(
            cores_per_link=2, fsus_per_core=8, fsu_width_ghz=12.5, core_qc_index=0, core_cc_index=1,
            xt_avoided=False,
        )
...
def test_network_reserved_cores_must_be_distinct():
    with pytest.raises(ValueError):
        NetworkConfig(
            cores_per_link=4, fsus_per_core=8, fsu_width_ghz=12.5, core_qc_index=1, core_cc_index=1,
            xt_avoided=False,
        )
...
def test_data_core_pool_excludes_reserved_cores():
    net = NetworkConfig(
        cores_per_link=5, fsus_per_core=8, fsu_width_ghz=12.5, core_qc_index=0, core_cc_index=2,
        xt_avoided=False,
    )
